...
const VideoUpload = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    description: '',
    duration: 0,
    resolution: '', // Add 'resolution' state
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (videoUrl) {
        await saveVideoInfoToDatabase(videoUrl);
        setMsg('Video uploaded successfully and information saved to the database.');
      } else {
      ...
      setMsg(error.response?.data.message || 'An error occurred while uploading the video.');
    }
  };
  ...
  const saveVideoInfoToDatabase = async (videoUrl) => {
    try {
    ...
    <div className="w-auto mt-16 ml-64 bg-[#282828] border border-[#3E3E3E] py-16 px-8 rounded-sm flex flex-col gap-2 font-prompt">
      <form onSubmit={handleSubmit}>
        ...
        <div className="w-full">
          {/* Input fields for title, description, duration, and resolution */}
          {['title', 'description', 'duration', 'resolution'].map((name) => (
            <div key={name} className="relative w-full flex flex-col items-start mt-2">
              <label className="text-sm" htmlFor={name}>
                {name.charAt(0).toUpperCase() + name.slice(1)} {/* Capitalize the field name */}
              </label>
              <input
